...
Patch `0074` gives protected scene-content and scene-actions calls an explicit 65,536-token task output ceiling, matching the existing formal paid authorization instead of requesting the model catalog's 393,216-token capability. At this patch, the Pro Agent retained its separate 16,384-token local allowance (superseded for observation mode by `0079`); models, prompts, interaction requirements, review gates, and ordinary unprotected workbench requests are unchanged. A formal reply that ends because of its output limit is archived under `data/formal-scene-provider-responses/` and rejected before page parsing or persistence, with no automatic retry and no deletion of an existing page. The patch changes no saved course or grant.

Patch `0075` resumes only the explicitly reviewed original sample session after its pre-dispatch budget rejection. It preserves the successful search and fetch results, original course identity, transcript, attempt history, and ledger, and requires the exact reviewed history digest plus active observation mode. It neither creates another classroom nor restores search attempts.

Patch `0076` honors the trusted accounting context's explicit `aggregateLimitsEnabled=false` by removing price-table per-request quota comparisons in Native observation mode. Default and legacy contexts retain those comparisons. Accounting still reserves, claims, and settles each real request; safe integer measurements, allowed units, exactly one call, scope checks, and unknown-outcome holds remain enforced. Model/task output settings and the actual SDK and media validation limits are unchanged; no grant, price, history, or quality requirement is rewritten.

Patch `0077` applies that same explicitly authorized observation mode to the separate formal Agent's cumulative dispatch and input-character counters. It awaits the authenticated context before the first dispatch, shares the original cached result or rejection with accounting, and keeps every old and new ledger entry. Only the 48-dispatch and 8M-character cumulative quota comparisons are skipped in observation mode; ordinary contexts, valid-input checks, zero retries, and teaching-quality gates are unchanged. This patch retained the 16,384-token local output allowance, later removed for observation mode by `0079`. This source patch does not modify a running session or ledger.

Patch `0078` removes the per-course four-search quota only when the trusted accounting context explicitly confirms observation mode, both before search dispatch and during final research acceptance. Old attempts remain recorded and cannot exhaust an observation-mode allowance. Successful searches still register their real URLs and emit research traces, while final acceptance still requires valid results, matching fetched source text, and classroom citations. Default-mode limits, search metering, provider failures, and the frozen generation policy remain unchanged.

Patch `0079` removes the formal Pro driver's self-imposed 16,384-token clamp only in trusted observation mode. This was a local allowance introduced by `0058`, not the model's technical boundary, and caused a later repair attempt to truncate. The driver now passes its existing catalog-backed `wireMaxOutputTokens` directly (currently 393,216 for DeepSeek V4 Pro), and accounting reserves that same request window while settling actual usage. Default-mode allowances, model context capacity, zero automatic retries, truncation rejection, and quality gates remain unchanged. This patch neither restarts the Agent nor modifies saved courseware.

Patch `0080` adds actual-renderer slide layout checks for glyphs/formulas escaping an established solid panel and text covered by a later opaque panel. Previously, auto-height text boxes could pass scroll and canvas bounds while their text appeared outside its background or was painted over. The checks use rendered geometry and the renderer's sibling paint order, excluding thin rules, translucent decoration, non-rectangular shapes and non-slide quiz/interactive layouts. Findings become normal browser-rejection issues through an optional metrics field, preserving old receipt shapes. Existing cached reviews are not relabelled; a repaired snapshot must be inspected again. These targeted checks supplement visual review rather than claim complete aesthetic validation.

Patch `0081` removes the cumulative three-review ceiling only when the trusted paid-budget context explicitly enables observation mode. It returns `remainingReviewAttempts: null` and `reviewAttemptLimitEnabled: false`; limited mode retains its original numeric count. Same-snapshot cache and unresolved-dispatch guards, original attempt records, and all quality requirements remain in force.

Patch `0082` captures exploration diagrams using the actual page coordinates of scaled iframe content. Chromium's element screenshot box omitted the host scale, so the old crop included neighbouring content and could falsely fail exact reset comparison. Screenshot and control probes now share the same checked projection through nested frames, including borders and page/frame scroll; unsupported transforms or clipped regions are rejected. PNG hashes, distinct-state checks, independent numeric/SVG verification and exact reset equality remain mandatory. No pixel tolerance, model call, generated-content change or old-receipt rewrite is introduced.
...
The professional Agent driver reserves each model turn durably before dispatch.
Default mode retains the local allowance of 48 turns per session, 8,000,000
cumulative serialized input characters and 16,384 output tokens per turn; old
recorded turns consume that allowance. These are self-imposed quotas, not model
technical limits. Trusted observation mode records every turn without those
quota checks and uses the driver's registered model output window (`0077`/`0079`).
SDK retries remain disabled; separate content, verification, media and publication
...
Patch `0083` adds the operator-only `complete-browser-recheck` mode for an unchanged saved snapshot whose original completion stopped at browser checks. It binds the original claim and failure bytes, the three inspector dependency hashes, and the same authorization's authoritative dispatch history. Unresolved or later paid calls block reuse. The old claim and rejection remain in the audit; the normal quality and audio pipeline still determines success. See the [single-course operator guide](../docs/architecture/2026-09-10-single-course-operator.md) for manifest fields and the separate backend reconciliation step.

Patch `0084` makes the formal math ASR comparison recognize equivalent written and spoken fractions, decimals and percentages, while rejecting changed mathematical values before applying the unchanged 8500-bps similarity threshold. It leaves speech text, audio identities and original ASR records intact.

Patch `0085` adds `revalidate-audio-comparison` and `complete-audio-comparison-tail` for a saved math ASR result rejected by the earlier comparator. The first command makes no provider call: it verifies the original WAV, transcript, speech text and primitive records, then saves a separate, hash-bound comparator receipt. The second verifies that receipt against the same session, saved job and classroom snapshot and requires already-passed quality evidence (`cachedQualityOnly`). It reuses successful audio records and generates only missing segments. The old ASR score remains in the final receipt's `comparisonRevalidation` source audit; no `operatorRepair` is invented. The same patch normalizes the equivalent denominator-100 forms “一百分之” and “百分之” for math comparison only.

Patch `0086` allows the existing `asrComparisonRevalidation` option to hold one reference or an array. Every reference is checked before any missing audio can be generated; duplicate ASR IDs or mixed session/job/snapshot identities are rejected. A sorted set digest binds the durable completion claim, and each final segment retains only its own full audit. It also extracts hard mathematical values before removing punctuation, preventing a preceding number across a comma from being mistaken for part of a fraction denominator. The 8500-bps threshold and rejection of changed mathematical values remain unchanged.

Patch `0087` changes only the saved-stage CLI. Both single-segment repair and diagnostic Provider calls require the original job's paid binding in a stable operation namespace. It neither releases nor zero-settles a previous unknown outcome. A comparison audio tail may additionally bind `audioRepair: { asrRequestId, receiptSha256 }` to a completed physical `.repair1` promotion. The CLI verifies the retained source, original rejection, physical and logical records, WAV, transcript, current classroom identity and reconstructed promotion before combining that evidence with the comparison references into a new claim. The original consumed claim remains required and intact. This CLI-only patch changes no server health contract or production bundle.

Patch `0088` extends that CLI-only `audioRepair` marker to one object or an array. Every successful repair journal must independently match the same frozen classroom; duplicate IDs and empty sets are rejected. The complete sorted set binds the continuation claim. Wrapping a single repair in an array preserves its original claim digest. Existing comparison receipts, physical repairs and unknown original outcomes remain intact; no comparator or release threshold changes.

Patch `0089` spells out mathematical notation only in new physical TTS Provider input. Logical narration, request hashes and classroom snapshots stay unchanged. The physical pronunciation policy, original/converted text hashes, actual character count and Provider body hash are persisted before dispatch and included in the paid request. An explicitly diagnosed math segment whose original and `.repair1` ASR both still fail can use the fixed `.pronunciation1` lane, bound to the exact failed records and a materially changed pronunciation input. Its separate source/promotion journal preserves both prior failures; the normal ASR threshold still decides success. The CLI accepts its final repair reference with `repairKind: 'pronunciation1'`, alongside existing repair references. No arbitrary attempt suffix or automatic retry is added.


Patch `0090` makes frozen student teaching discussions mandatory: playback opens them automatically, keeps failures and manual stops at the same discussion for an explicit retry, and advances pages only after the backend accepts scene completion. Read-only progress synchronization restores unfinished teaching without first flushing its rejected completion; accepted scene IDs remain scoped to the same learning session across legitimate re-entry. Optional operator discussion cards retain their previous behavior. The required Pi teacher roster now receives its registered role actions before capability inspection; configuration preflight precedes turn claims. The one diagnosed empty role-configuration failure can resume under the same conversation only when trusted accounting proves no actual or unknown dispatch, preserving the complete failure archive and authorization identity.


Patch `0091` requires visible teacher explanation during authenticated required teaching: highlighting or other actions alone cannot satisfy cue-user, close-session or end-of-loop fallback. A missing explanation returns explicit tool feedback for one local repair; another empty result ends that turn as a failure without claiming teaching or learner participation. Ordinary OpenMAIC discussion behavior is preserved. A diagnosed initial silent turn may retry under the same conversation and authorization only after a definite done event, the exact incomplete-teaching error, no learner response or spoken/audio output, and trusted accounting confirms all usage is settled. The complete failed record is archived by revision before the new revision starts; original charges and failure evidence remain intact.

Patch `0093` restores the avatar-anchored invitation before required classroom discussions. The learner explicitly joins; required invitations have no countdown or skip control, and neither a stale skip callback nor exhausted narration can satisfy discussion completion. Explicit retries keep the same frozen action. In the student orbital theme, dedicated arrow gutters and avatar padding prevent hover controls from obscuring portraits; an invited companion is scrolled into the avatar strip without moving the classroom page. Optional operator discussions retain their countdown and skip behavior.

Patch `0094` adds an opt-in AI-designed playful learning policy for new formal courses. The Pro Agent chooses the game rules and representation from the frozen lesson brief; no prebuilt game or level template is supplied. New courses require a replayable learning game and group the unchanged four assessment questions into two quiz pages. Appropriate mathematics briefs require a manipulable teaching model in 3D. Existing frozen policies remain valid, and structural checks do not replace semantic, browser, audio or publication review. See [AI playful courseware](../docs/architecture/2026-09-11-ai-playful-courseware.md) for the contract and actual sample status.

Patch `0095` projects that frozen policy directly into each page's content/actions calls and independent teaching review. New game prompts ask the AI to choose its own non-quiz rules with zero official scoring; actual generated configuration remains AI output. The context binds generation receipts, review input hashes and cache identity. A prior review lacking that context cannot be reused or silently reset. Historical prompts and cache paths remain unchanged.

Patch `0096` adds repair locations to exploration contract errors and scene ordering diagnostics. The Pro Agent receives the missing numeric state fields or offending demonstration/operation positions, while all existing validation rules and error prefixes remain in force. This improves self-repair without accepting incomplete teaching evidence.

Patch `0097` projects only known `message_update` streaming frames out of the formal evidence reader, retaining every structural and unknown event, all source PostgreSQL history, the 20,000 structural-event limit, monotonic cursors and a fixed high-water mark. Legacy event reads remain unchanged. This prevents long token streams from blocking stage, research and skill receipt verification.

Patch `0098` keeps the exact reset comparison and adds a narrow fallback for independently verified numeric SVGs: the SVG, numeric values, feedback, bounds, image size and alpha must match. Only existing high-contrast edges may differ by one RGB level, bounded by both 64 pixels and 0.05 percent of the image. Other models and changes to values, feedback, shape or material pixels still fail. No generated course HTML or assessment is edited.

Patch `0099` makes the operator-only saved-stage browser inspection reconstruct the same frozen grade, question identities and interaction policy used by ordinary formal review. It executes the existing declared teaching probes and multistate verification instead of treating a click on an initial-state reset as a state-changing action. Legacy courses retain generic probing; invalid declared contracts still fail. The CLI was first verified directly against the 0098 deployment and is now included in the 0100 production build.

Patch `0100` accepts an independently audited completion budget only through `complete --completion-paid-budget <sidecar.json>`. The internal accounting service must authenticate the complete sidecar before the private completion wrapper can use it. The original job, input, session, stage, locked questions and speech identities remain fixed. This path cannot start an Agent or mix other recovery modes, and records the completion grant hash alongside its claim and receipt. Source integration alone does not mean the saved draft has passed review or been published.

The managed production build through `0100` passed on 2026-09-11, with BUILD_ID `8b3XT59hYcNl-nO6rh3iF`; Native, Gateway and Backend restarted successfully. The actual completion grant apply was rejected by automatic approval before execution. No completion credential or paid tail calls were issued, and the course remains unpublished pending explicit confirmation of that scope.

Patch `0101` adds explicit `complete-learner-discussion-repair` after an authentic failed completion. A single DeepSeek Flash request generates two teacher-led learner discussion additions, preserving all original scenes, games, assessments, speech and other actions. The original claim remains; a distinct claim and raw Provider audit bind the source, rejection, input and derived output. The existing verified completion budget and full final review/audio gates remain required. The 2026-09-14 user confirmation allowed the original completion, whose visual check passed but pedagogical review found these missing learner explanation opportunities; the initial two paid reviews are settled and retained.

The managed build through `0101` passed with BUILD_ID `k9ZOyt7V8mNCSIvwtJZRH`. Automatic approval rejected the actual discussion-repair execution before it ran, requiring explicit approval for the three additional expected DeepSeek calls. No discussion repair, TTS, ASR or publication occurred. The remaining proposed execution is one AI repair, two reviews and the original 36 TTS/36 ASR segments; including the two settled initial reviews, the estimate is 77 calls.

Patch 0102 recovers the unchanged, already settled learner-discussion reply after a literal vocabulary validator incorrectly rejected an otherwise explicit classroom-input invitation. Six frozen source hashes and append-only recovery evidence are required; the recovery calls no Provider. The actual derived snapshot remains subject to ordinary review.

Patch 0103 renders quiz choices in two columns on wider displays and one column on narrow displays. The disposable inspector retains local interaction evidence but captures a stable, reloaded unanswered quiz, so a selected distractor or probe scroll position is not mistaken for authored teaching content. Explicit derived-snapshot re-review uses the existing review-grant audit and verified completion authority. Trusted observation mode permits a nonempty, fully hashed prior review ledger, while strict mode retains the exact-three-attempt requirement. No course text, locked question, original AI game, original failure, reservation or authorization is rewritten.

Patch 0104 fixes tsx keepNames serialization of the private quiz screenshot callback without injecting browser globals, and removes residual pointer hover. The managed build passed with BUILD_ID wWSwpM-H2K0nRA5OaVMjN. Four real quiz viewport checks passed. The actual final re-review execution was rejected before process creation by automatic approval because the new visual call raises the estimate from 77 to 78; that one additional paid vision call requires explicit user approval. No new re-review claim, TTS/ASR or publication occurred.


Patch `0105` adds a one-time operator tail for a reviewed learner-discussion repair interrupted during formal audio. It verifies both frozen snapshots, retained claims, cached quality, all prior WAV/record hashes and the exact failed segment before reusing 22 completed pairs and invoking the existing physical repair/lifecycle for the remaining 14. A separately audited expiry-only renewal is accepted for the same completion authority; original unknown outcomes and consumed claims remain intact. On 2026-09-14 the real tail completed all 36 TTS/ASR pairs with 28 new settled audio calls and no new quality-model calls. See `docs/architecture/2026-09-14-saved-stage-audio-tail.md` and the final validation output for subsequent publication and runtime verification.

Patch `0106` scopes the Orbit palette to sidebar conversation bubbles, speaker labels, avatars, session borders/badges, the active message marker and the continue-discussion control. Semantic hooks cover teachers stored with either teacher or agent roles and suppress the upstream inline purple glow in student chat. Operator colors, discussion behavior, authored courseware and API contracts are unchanged.

Patch `0107` distinguishes real learner replies from virtual classmates at the authenticated teaching execution boundary, including saved conversation continuations and both Native and Legacy teacher runtimes. Student discussions offer explicit join/skip choices and no longer redirect navigation to unfinished discussions. Exploration participation accepts actual changed values and visible feedback from declared controls instead of requiring publication sample answers. Independent quizzes remain authoritative. See `../docs/architecture/2026-09-15-classroom-speaker-identity.md`.

Patch `0108` aligns desktop discussion choices, recovery controls, history actions and playback controls to 36 px with 48 px coarse-pointer targets. Semantic avatar, collapsed-message and response-cue hooks remove the remaining purple chrome in student discussion, including user messages. Student primary/secondary/end actions retain distinct colors; dedicated voice controls stay large. Operator styling, model routing, authored courseware and discussion behavior are unchanged.
