...
def task_service() -> TaskService:
    return TaskService(current_app.config["DATABASE_URL"], auth_service=auth_service())
...
def camera_bridge_service() -> CameraBridgeService:
    adapter_name = str(current_app.config.get("CAMERA_RUNTIME_ADAPTER", "disabled")).lower()
    if adapter_name == "ai_camera_test":
        _require_dev_adapter("APP_CAMERA_RUNTIME_ADAPTER=ai_camera_test")
        base_url = current_app.config.get("CAMERA_BACKEND_URL")
        if not base_url:
            raise ApiError(
                "camera_runtime_not_configured",
                "开发摄像头桥接已启用，但 APP_CAMERA_BACKEND_URL 未配置。",
                503,
            )
