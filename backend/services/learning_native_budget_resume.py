"""Exact original-instance continuation after the reviewed pre-dispatch budget refusal.
...
EVENT = 'single_course_native_budget_resume'
...
from services.learning_native_prestage_recovery import (
    _admitted_preview_from_archive, _public_recovery_event,
)


def _settled_ledger_sha256(ledger):
    if any(row['state'] in {'reserved', 'dispatched', 'unknown'} for row in ledger):
        raise ValueError('Budget resume requires every original dispatch outcome to be settled')
    if any(row['state'] not in {'settled', 'released'} for row in ledger):
        raise ValueError('Budget resume encountered an unsupported accounting state')
    return digest(sorted(ledger,key=lambda row:row['id']))
...
def _native(config, mode, expected=None):
    ...
    response = client._request_json('POST', '/api/generate-classroom/' + JOB + '/budget-resume', body)
    result = response.get('recovery')
    if response.get('ok') is not True or not isinstance(result, dict) or result.get('jobId') != JOB:
        raise ValueError('Native budget resume response invalid')
    return result


def _prepare_single_native_budget_resume(*, adapter, budget, config, identity, expected_history_sha, now, apply=False, native=_native):
    ...
    saved_path = ARCHIVES / ('native-budget-resume-' + str(expected_history_sha) + '.json')
    ...
    with repository.transaction() as conn:
        # Same lock as accounting admissions; do not increment its revision for a plan.
        ...
        owners = list(conn.execute('SELECT * FROM learning_curriculum_preparation_plans WHERE catalog_build_id=? '
            'AND target_fingerprint=? AND library_target_fingerprint=? FOR UPDATE',
            (build['id'],identity['targetFingerprint'],identity['targetFingerprint'])).fetchall())
        ...
        if pending_event:
            ...
            if (not apply or pending.get('recoveryReceiptId') != expected_history_sha or not saved
                    ...
                    or runtime['status'] != 'recovering' or runtime['error_code'] != 'openmaic_formal_budget_resume_pending'
        ...
        if (runtime['status'] != 'failed' or runtime['quality_status'] != 'rejected' or runtime['error_code'] != 'openmaic_formal_generation_failed'
            ...
            raise ValueError('Only the original budget-refused sample can be reopened')
        ...
        ledger = list(budget.repository.authorization_ledger(conn, binding.get('authorizationId')))
        ledger_sha = _settled_ledger_sha256(ledger)
        observation_mode = budget.policy.raw.get('aggregateLimitsEnabled') is False
        if apply and not observation_mode:
            raise ValueError('Budget resume requires explicitly enabled observation mode')
        ...
        if (native_proof.get('runtimeRequestId') != runtime['request_id'] or native_proof.get('buildItemId') != item['id']
                ...
                or native_proof.get('qaCount') != 0 or native_proof.get('remainingSearchAttempts') != 0):
            raise ValueError('Native and Backend frozen sample identities do not match')
        audit = {'schemaVersion':'mira.single-course.native-budget-resume.v1','originalOwner':dict(owner),
            ...
            'native':native_proof,'budgetLedgerSha256':ledger_sha,
        ...
        preview = {'historySha256':history,'nativeHistorySha256':native_proof['historySha256'],'jobId':JOB,
            ...
            'remainingSearchAttempts':0,'searchApiConfigured':search_configured,
            'contentDispatches':len(dispatches),'budgetContextValid':budget_valid,'applied':False,'aggregateLimitsEnabled':not observation_mode,
            'blockers':([] if search_configured else ['search_api_key_required']) + ([] if budget_valid else ['original_budget_inactive'])
                + ([] if observation_mode else ['aggregate_limits_must_be_disabled'])}
        ...
        changed = conn.execute("UPDATE learning_openmaic_runtime_classrooms SET status='recovering',quality_status='pending_review',"
            "error_code='openmaic_formal_budget_resume_pending',error_message_safe=NULL,updated_at=? WHERE id=? AND status='failed' "
...
def resume_single_native_budget_failure(*, adapter, budget, config, identity, expected_history_sha, now, apply=False, native=_native):
    prepared = _prepare_single_native_budget_resume(adapter=adapter,budget=budget,config=config,identity=identity,
    ...
    with repo.transaction() as conn:
        ...
        if (not row or not owner or row['status'] != 'recovering' or row['upstream_job_id'] != JOB
                or row['error_code'] != 'openmaic_formal_budget_resume_pending' or owner['status'] != 'running'):
