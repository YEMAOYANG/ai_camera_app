...
2026-09-10 需求复核：补充适龄深度交互作为新课默认设计、教学操作证据、课库撤回与开放范围结束状态，并纠正文档优先级。详见[需求与官方深度交互复核](2026-09-10-requirements-deep-interaction-review.md)。这些是待落实的生产/验收合同，不能描述成已修改运行默认值或已升级现有三门课。
...
注册保持昵称和年级的必要信息，不新增家长填写备课提示词、选择模型或先完成诊断的门槛。可选诊断不阻挡首课。三科学科覆盖、首批三门课与每天推荐几门课是不同配置；日推荐数量/学习时长应由同一服务端策略和已确认需求决定，家长端与学生端不能分别写死两课/三课。等待文案使用“老师正在认真备课”等中性表述，产品命名未定前不新增品牌化角色称呼。
...
库存只计算仍有效、已发布、媒体可用且对当前路径适用的版本。撤回错误课或媒体失效会重新形成缺口；修复任务使用新的版本/尝试身份，保留原审核与费用审计，并使关联缓存失效。已开始的会话按缺陷严重性明确保留、暂停或迁移，不能覆盖原文件悄悄换课。

孩子学完当前开放范围后，展示“本单元已完成”，再明确下一单元未开放或确实正在准备；巩固内容标为复习。全库有课但孩子当前路径没有合法下一课时，也不能用全库数量冒充个人可学库存，更不能永久假装备课。
...
官方深度交互包含模拟、游戏、图解、3D 和编程等动手学习形式，教师可操作界面作引导。[官方功能说明](https://github.com/THU-MAIC/OpenMAIC#deep-interactive-mode-new)

正式新课拟使用版本化 `interactionDesignPolicy`：`enabled=true`、`profile=primary-adaptive`，覆盖规划、制作、质量审核和发布。它要进入新任务的内容指纹；旧任务和旧课保留原策略，修改网页首页开关或补写 `stage.interactiveMode=true` 不算完成集成。

每个核心教学目标都要映射到一条适合该内容的“示范 → 学生操作/选择/组织 → 可观察结果 → 理由反馈 → 独立判断”学习链。允许一条学习链覆盖多个相关目标，不为凑数量强行加页；材料阅读、识字讲解和必要总结可以保留静态呈现。

- 数学可用摆数量、调整参数、比较结果、拆分组合；语言课可用分类、排序、材料证据选择和语义匹配。活动要改变与知识点有关的状态，纯翻页/播放按钮不算教学操作。
- 三维适用于空间目标，编程适用于明确相关目标。不能要求每门小学语数英课程集齐全部五种交互形式，也不能让低年级承担不适龄操作负担。
- 保留老师预生成示范动作，给孩子独立操作机会；自动示范结束不能替孩子完成必需操作或测评。
- 检查正确路径、错误反馈、重试、复位、暂停/恢复及触屏可用性。已有单个浏览器探针只能证明部分控件可运行，需要补齐教学链验证。
- 必需操作的完成条件写入课件合同，与固定场景/操作 ID 和版本绑定；由现有授权的课堂事件通道记录证据。后端检查会话、题目/场景和完成规则，不能直接相信浏览器给的分数或 `mastered=true`。
- 用单独客观测评判断知识掌握，操作完成只是学习证据的一部分。阅读欣赏等开放学习结果按明确范围表达，不伪造确定性评分。

所固定官方版本的 Pro `deep-interactive` skill 还含至少 60% 互动页、开场 slide、相邻交互形式轮换等结构约束；这些用于大纲提示及工具结果诊断，由 Agent 决定是否重规划，并非官方统一发布硬门槛。本地正式课程目前未强制采用这些规则。推荐默认策略是适龄深交互，不能将其宣传成所有结构规则与官方模式完全相同。需要官方结构对照时用独立配置与样课验收，不把比例作为小学课程质量的唯一依据。

### 分开验收三类交互与费用

1. **课件操作**：已生成的 HTML/控件、预生成示范和音频默认可用；这些操作本身不触发新的语言模型调用，实时问答额度不足不能关闭它们。
2. **必要实时指导**：确需模型按孩子当前情况响应的步骤，在课前准入并预留预算；不能承诺预生成课件就自动覆盖所有个别追问。
3. **可选延伸对话**：额外讨论、自由追问及其产生的新增语音按需计费，保持独立额度，不因默认深交互而自动无限启动。必要实时指导所需的新语音仍计入必要教学预算。

每门课记录“所选策略、实际技能读取、研究来源、互动产物、教师动作、学生操作验证、在线调用结果”各自状态。不能以技能名存在、徽标亮起或浏览器探针通过代替全部真实学生流程验收。
...
| 深度交互设计与证据 | 新版交互策略；正式 Pro 的规划/课堂校验/scene inspector；后端发布 gate 与 runtime event 服务 | 新任务默认适龄深交互；验证核心目标的教学链，区分自动示范、学生操作与独立掌握；旧指纹继续兼容 |
...
- **课程目录/准备摘要**：`gradeCode`、`curriculumVersion`、个人适用的 `availableCourseCount`、`availabilityStatus`（ready/preparing/paused/not_open/scope_completed）、`generationProgress`、`retryAfterMs`。目录总数另计；有课可学时仍可内部记录生产状态，前台不显示进度条。
- **课堂启动**：固定 `lessonVersion`、`interactionDesignPolicy`、必需教学步骤、`interactionCapabilities` 及服务端预留引用；能力分别描述课件操作、必要实时指导、可选对话。预算资格由服务端派生，不相信浏览器传入金额或用户身份。
- **教学操作证据**：通过既有会话事件通道扩展操作 ID、场景版本、操作来源、可校验结果和顺序；区分老师演示与学生操作，重复/过期/跨会话事件不能推进完成。上线前先验证事件与实际课堂控件的绑定，不把任意前端状态当成权威成绩。
...
| 2 | 一至六年级合同与全链身份隔离；接入默认深交互政策及新旧兼容；同时补真实调用记账和统一预算控制 | 无付费 fixtures 验证年级/缓存/调度隔离、错误题与假交互拒绝、多 worker 抢占预算和一年级旧课兼容；未来任务选用新政策而旧指纹不变；付费入口均受控后再扩库 |
| 3 | 补一年级下一知识点与巩固路径；二年级三科小批试课 | 学完首课能进入合法新内容，未掌握有合适巩固；至少一门数量/机制课和一门语言材料课完成默认深交互真实验收；二年级制作到家长报告通过；付费批次总额和次数明确 |
...
11. 默认深交互政策进入新任务、实际课件和发布回执；核心教学目标有有效学习链，老师示范不能代替学生操作，学生操作不能代替知识掌握。
12. 错误内容撤回会使适用库存失效并按授权范围补缺；开放范围学完显示真实完成/后续开放状态，不以复习冒充新课。
