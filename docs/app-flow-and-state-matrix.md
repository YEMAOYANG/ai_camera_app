...
来源：`README.md`、`docs/grade_based_kindergarten_primary_learning_plan.md`、`docs/ai_camera_parent_needs_product_tech_plan.md`、`design/mira_guardian_high_fidelity_ui.html` 的业务页面/交互状态
...
| `parentIdentity` | 家长身份 | 主流程 | 登录后 | HTML/README | 第 1 步；妈妈/爸爸/祖辈/其他家人 |
| `bind` | 扫码绑定 | 二级流程 | 首页/我的 -> 设备管理 | HTML/README | 二维码、蓝牙发现 |
| `wifi` | Wi-Fi 配网 | 二级流程 | 发现设备后 | HTML/需求补齐 | 需要 loading/error/offline |
| `bindDone` | 绑定成功 | 二级流程 | 配网成功 | HTML | 返回设备管理或首页 |
| `child` | 孩子昵称与年级 / 孩子资料 | 主流程/二级 | onboarding/我的 | HTML/README | 第 2 步同时必填昵称与年级并原子提交；编辑态可修改 |
| `name` | 摄像头命名 | 二级流程 | 设备管理 | HTML/产品方案 | 设备名/唤醒名独立；口播和语音优先使用孩子昵称 |
| `contacts` | 紧急联系人 | 二级流程 | 我的/安全 | HTML/README | SOS 兜底联系人 |
...
- 孩子常用昵称去除首尾空格后不能为空；年级默认不选择，任一字段缺失时“确定”禁用。
- 年级只显示幼儿园 3 项和小学 6 项，学段由年级派生且只允许幼儿园或小学。
- “确定”在一次请求中提交昵称、年级编码和学年；服务端原子保存，不能出现单字段成功，保存中不可重复提交。
- 保存失败保留昵称和年级选择；中断后恢复本地暂存或服务端已保存的完整组合。
- 首次设置完成后，摄像头口播和语音对话优先使用已保存的孩子昵称；设备名和唤醒名不替代孩子昵称。
- 摄像头绑定、命名、紧急联系人和权限授权均从首页或“我的”按需进入。
...
| 家长身份 | 角色选择、显示名 | 保存中 | 未选角色 | 保存失败 | 进入孩子昵称与年级 | N/A | N/A |
| 孩子昵称与年级 | 必填昵称、9 个年级选项 | 原子保存昵称、年级和学年 | 昵称为空或未选年级时不可提交 | 保存失败并保留两个字段 | 完整组合保存、派生幼儿园/小学学段并进入首页 | 无权编辑 | N/A |
| 扫码绑定 | 扫码框、蓝牙发现 | 扫码识别、蓝牙扫描 | 未发现设备 | 二维码无效 | 进入配网 | 相机权限拒绝 | 设备待配网 |
| Wi-Fi 配网 | SSID、密码 | 配网中 | 未发现网络 | 密码错误、超时 | 绑定成功 | N/A | 弱网、配网失败 |
| 绑定成功 | 设备成功卡 | 同步设备信息 | N/A | 同步失败 | 返回设备管理或首页 | N/A | 在线/待校准 |
| 孩子资料（编辑态） | 已保存昵称、年级与可选资料 | 保存中 | N/A | 字段错误 | 保存成功 | 无权编辑 | N/A |
| 学生学习空间登录 | 首页右上角扫码、小学孩子、网页二维码与 4 位核对码 | 正在读取浏览器登录请求 | 非小学年级不开放 | 二维码无效、核对码不一致、PIN 不一致、网络失败 | 显示浏览器/系统/孩子，首次设置 4 位 PIN；批准后等待网页完成交换 | 无 `manage_child_profile` 或相机权限 | 二维码 5 分钟过期；无法扫码时使用 8 位配对码兜底 |
| 摄像头命名 | 设备名/唤醒名、试听 | 保存/试听 | N/A | 识别失败 | 命名成功 | 麦克风权限拒绝 | 设备未连接时只保存 App；孩子称呼仍优先读昵称 |
| 紧急联系人 | 联系人列表 | 邀请中 | 暂无联系人 | 邀请失败 | 联系人已添加 | 无通讯录权限 | N/A |
| 权限引导 | 通知/相机/麦克风说明 | 请求权限 | N/A | 权限请求失败 | 授权完成 | 权限拒绝 | N/A |

学生登录权限与课程可播放状态独立。已保存开放年级（当前服务端为一年级）且 `grade_selection_revision >= 1` 的孩子，即使可用课程为 0，也可扫码或使用配对码登录网页查看准备进度。首页扫码、扫码页、孩子资料入口和备用配对码页统一使用服务端 `canAccessWorkspace`；课程展示和播放仍使用 `canLearnNow` 及原有发布校验。`GET /api/learning/availability` 增加可选查询 `includeWorkspaceAccess=true` 时返回布尔字段 `availability.canAccessWorkspace`，未传时保留旧版响应字段，兼容严格解析响应的旧 App。家长权限、孩子归属、二维码有效期和 PIN 校验继续由原登录接口执行。

扫码和配对码流程使用独立的 `studentAccessAvailabilityProvider`，按孩子、年级、学年和年级确认时间隔离；进入流程时查询权限，首页课程轮询不刷新该状态。扫码预览、批准和创建配对码仍由服务端核验最新权限。相机启动/停止必须幂等，重复路由或应用前台通知不应重启已运行的相机。

只有当前没有任何可学习课程时，家长和学生端才展示课程准备百分比及进度条；一旦有首门可学课程，进度区域随状态更新自动隐藏，后续补课库不再展示百分比。家长端读取现有 `progressPercent`；学生端今日课程/课程库接口增加可选可空的 `preparationProgressPercent`（0–100），来自当前孩子、年级和 `grade_selection_revision` 对应且未被替代的准备计划。没有计划进度时显示正在获取，不编造百分比；未完成阶段最高显示 99%，完成才显示 100%。百分比代表整批新课准备工作，不是等待时间或单节课的进度；每完成一节可以先学。准备中的学生卡片使用“老师正在认真备课”等文案，不把尚未确认的项目名称当作老师称呼。

### 4.2 首页与任务状态矩阵

小学学习 V1 只开放 1—6 年级语文、数学、英语三科。生产环境中大多数年级学科各有 3 个能力边界，一年级语文为保证从 a/o/e 起步而细分为 4 个，共 55 个能力边界；边界只定义目标、允许内容、排除内容和先修要求，不包含固定题，也不是题库。一、二年级英语属于 Mira 拓展内容，不代表国家统一开设要求或与当地教材同步。当前能力边界仍不是完整课标或完整学年覆盖。运行时只使用白名单确定性评分；开放式作文、朗读、口语等主观评分不上线。

每天 05:00 根据 14 天矩阵确定三科顺序，并为孩子检查对应 `gradeCode` 下尚未学习的语文、数学、英语动态课程。缺课时，将 `gradeCode`、`subject`、`skillBoundary` 交给 OpenMAIC generation 和 Kimi，生成 `teachingFlow` 加 5 个已验证题对象的“先教后练”候选：`q1` 为不计分示范，`q2`—`q3` 为互动引导，`q4`—`q5` 为独立练习，运行时只计 `q2`—`q5` 共 4 题。后端再构造不含答案字段的 `workedExample={questionId:'q1',explanation}`；另一次 fresh Kimi 验证只看公开讲解、示范解析和公开题面，除独立解题外还返回 `teachingReview`。课堂展示链由 OpenMAIC 生成答案盲的 `classroom_intent.v2` 五段意图，Python 只用白名单模板编译课件，拒绝模型输出的 HTML、Canvas、JavaScript、URL、动作和答案。只有独立复核、确定性闸门、课堂发布门和数据库唯一约束全部通过，候选才会发布为 `content_origin=openmaic_generated`。当日任务只选择该来源；模型不可用或任一校验失败时保持未发布，也不回退固定题。
...
| 小学今日学习 | 家长 App 显示进度与报告；Student Web 每天显示语文、数学、英语三节动态年级课程 | 获取当天三个 `learning` 任务；无未学动态课时生成、fresh 解题和校验；缺失时由 `GET` 幂等兜底 | 课程仍在生成或尚无通过校验的 `openmaic_generated` 课程 | OpenMAIC / Kimi 不可用或校验失败；可重试但不发布、不回退固定题、不重复建任务 | Student Web 显示三节课程、学科和默认开课时间 19:30 / 19:45 / 20:00 | 学生 token 只可访问绑定的孩子；家长权限按家庭校验 | N/A | Student Web 不依赖摄像头在线；家长端保留协助回退 |
| Student Web 课程 / 家长协助回退 | 受控五段课堂：讲解、示范、引导、独立检查、总结 | 启动或恢复同一学习会话；讲解后展示 `q1` 示范，再提交 `q2`—`q5` 的孩子回答 | 会话缺少已发布受控 LessonPackage 或题对象 | 请求失败时保留当前步骤和答案并重试 | `q1` 不计分；q2/q3 必须真实判题后才能推进；q4/q5 首答 2/2 正确才声明本节掌握 | 学生 token 不能越权访问兄弟姐妹或家长 API | N/A | 当前不依赖摄像头在线；未来摄像头执行同一会话 |
| 小学学习报告 | 首次独立答对、提示次数、掌握状态、下次建议 | 获取本次或最近报告 | 尚未完成学习 | 报告加载失败 | 基于真实答题记录持久化 | 无权查看该孩子 | N/A | 不依赖摄像头在线 |

能力点顺序属于后端确定性课程规则，不交给 OpenMAIC/Kimi 决定。新孩子从每科第一个基础能力点开始，未达到 `mastered` 时继续同一能力点的动态变式，达到后才解锁下一能力点；一年级语文固定为“单韵母 a/o/e → 声母与简单音节 → 汉字与词语 → 完整句子”。无学习会话且尚未开始的旧错误任务可保留任务 ID 安全替换；已有会话或已完成记录不改写，下一次同科学习回补正确的前置能力点。
...
| 自动准备今日学习 | 后台每天 Asia/Shanghai 05:00 自动触发；服务恢复后补跑，App `GET` 兜底 | 按 14 天矩阵调整先后顺序，但每天固定覆盖语文、数学、英语；无未学动态课时依次执行 OpenMAIC + Kimi 生成 `teachingFlow` 与 5 个题对象、补充无答案字段的 `q1` 示范解析、fresh Kimi 独立解题与 `teachingReview`、其余质量闸门 | 同日同槽位已有任务时幂等复用；生成中不创建空任务 | 模型不可用、`teachingReview` 不通过或任一校验失败时不发布、不回退固定题；保留安全重试 | 只引用 `content_origin=openmaic_generated` 的已发布课程创建三节任务；默认 19:30 / 19:45 / 20:00 开课 | 年级/昵称缺失时不触发 | 每天语文、数学、英语各一节；能力边界未掌握时不前进，生成时间与开课时间分离，不要求家长每天设置 |
| 执行先教后练课程 | “老师先讲”按钮 / 自动进入 | 加载讲解、示范与当前练习步骤 | 缺少已发布课程内容 | 保留会话进度并允许重试 | 老师先讲 → 看 `q1` 示范 → 做 `q2`—`q3` 互动引导 → 做 `q4`—`q5` 独立练习 → 总结 | 无权访问该孩子 | 当前由家长协助；未来摄像头按同一步骤执行 |
| 提交孩子答案 | `q2`—`q5` 提交按钮 pressed | 程序判题中 | 答案为空 | 正确后进入下一道计分题，4 题均答对后进入总结与报告 | 格式错误或服务失败 | 答错后留在本题：首次给提示，后续重新讲解并继续重做，直到答对；`q1` 示范不提交、不计分 |
